...
Overview
This project implements a password-based door lock system using the AT89C51 microcontroller. The system allows secure access through a 4x4 matrix keypad, displays status messages on a 16x2 LCD, and controls a motor or buzzer using the L293D motor driver IC. The project simulates a real-world security system designed for homes, offices, or restricted areas.
...
Optional: After multiple wrong attempts, the system can lock out for added security.

Circuit Diagram
The complete circuit diagram is designed in Proteus and includes:
...
How to Run
Open the Proteus project and load the provided schematic.
...
Observe the LCD for system messages and motor/buzzer action.

Future Enhancements
Add password change functionality.

Introduce a lockout mechanism after multiple wrong attempts.

Integrate a real-time clock (RTC) for time-based access.

Use EEPROM to store user passwords for persistent storage.
